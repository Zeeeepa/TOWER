...
class BrowserContext:
    ...
    def _execute_step(self, step: Dict[str, Any]) -> None:
        ...
        # ==================== INTERACTION ====================
            ...
            self.hover(get("selector"), duration=get("duration"))
    ...
    def remove_network_rule(self, rule_id: str) -> None:
        ...
        """
        self._cmd("removeNetworkRule", rule_id=rule_id)
    ...
    def get_network_log(self, limit: Optional[int] = None) -> List[NetworkLogEntry]:
        """
        ...
        """
        result = self._cmd("getNetworkLog", limit=limit or 0)
        if not result:
            return []

        entries = result if isinstance(result, list) else json.loads(result)
        return [
            NetworkLogEntry(
                url=e.get("url", ""),
                method=e.get("method", ""),
    ...
    def wait_for_download(
        ...
        Args:
            download_id: Specific download ID to wait for (optional)
            timeout: Timeout in milliseconds (default: 30000)
    ...
    def cancel_download(self, download_id: str) -> None:
        ...
        """
        self._cmd("cancelDownload", download_id=download_id)
    ...
    def hover(self, selector: str, duration: Optional[int] = None) -> None:
        """
        Hover over an element without clicking.

        Args:
            selector: CSS selector, coordinates (e.g., "100x200"), or semantic description
            duration: How long to hover in milliseconds (default: 100)
        ...
        Example:
            ...
            page.hover(".info-icon", duration=500)
            ```
        """
        self._cmd("hover", selector=selector, duration=duration)
    ...
    def keyboard_combo(self, key: str, modifiers: List[Union[ModifierKey, str]]) -> None:
        """
        Press a keyboard combination (e.g., Ctrl+A, Ctrl+C, etc.).

        Args:
            key: The main key to press
            modifiers: List of modifier keys ('ctrl', 'alt', 'shift', 'meta')

        Example:
            ```python
            # Select all with Ctrl+A
            page.keyboard_combo("a", [ModifierKey.CTRL])

            # Copy with Ctrl+C
            page.keyboard_combo("c", ["ctrl"])

            # Paste with Ctrl+V
            page.keyboard_combo("v", [ModifierKey.CTRL])
            ```
        """
        mod_values = [m.value if isinstance(m, ModifierKey) else m for m in modifiers]
        self._cmd("keyboardCombo", key=key, modifiers=mod_values)
    ...
    def evaluate(
        ...
        Example:
            ...
            ```
        """
        return self._cmd("evaluate", script=script, args=args, return_value=return_value)
    ...
    def upload_file(self, selector: str, file_paths: List[str]) -> None:
        ...
        Example:
            ...
            ```
        """
        self._cmd("uploadFile", selector=selector, file_paths=file_paths)
