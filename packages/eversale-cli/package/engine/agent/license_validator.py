...
async def validate_license() -> Tuple[bool, str]:
    """
    Main validation function. Checks license and returns (is_valid, message).

    This MUST be called before running any Eversale commands.
    """
    # Get license key
    license_key = get_license_key()
    if not license_key:
        return False, _get_no_license_message()

    # Check cache first
    cached_result = _check_cached_license(license_key)
    if cached_result:
        return cached_result

    # Validate online
    is_valid, message, user_data = await validate_license_online(license_key)
    return _process_validation_result(is_valid, message, user_data, license_key)


def validate_license_sync() -> Tuple[bool, str]:
    """
    Synchronous license validation - safe to call from anywhere.

    This uses synchronous httpx to avoid event loop issues when called
    from within asyncio.run() or other async contexts.
    """
    # Get license key
    license_key = get_license_key()
    if not license_key:
    ...
    # Validate online using synchronous httpx
    is_valid, message, user_data = validate_license_online_sync(license_key)
    return _process_validation_result(is_valid, message, user_data, license_key)
