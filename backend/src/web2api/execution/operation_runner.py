...
Optimized for Web2API chat interface interactions.
"""
...
class OperationRunner:
    ...
    For Web2API, this primarily handles chat_completion operations
    using the ChatExecutor for reliable web chat interactions.
    ...
    async def execute_operation(
        ...
        try:
            # Check if this is a chat_completion operation (primary Web2API use case)
            if operation_id == "chat_completion":
            ...
            # Legacy operation execution path for other operation types
    ...
    async def _execute_chat_completion(
        ...
        """
        Execute chat completion using the optimized ChatExecutor.
        
        This is the primary Web2API flow:
        1. Navigate to chat URL
        2. Type message into input
        3. Click send button
        4. Wait for response (track button state)
        5. Extract and return response
        """
        message = parameters.get("message", "")
        if not message:
            raise ValueError("No message provided for chat completion")
        
        # Get UI selectors from service config
        ui_selectors = service_config.get("ui_selectors", {})
        
        input_selector = ui_selectors.get("input")
        submit_selector = ui_selectors.get("submit")
        output_selector = ui_selectors.get("output")
        
        if not all([input_selector, submit_selector, output_selector]):
            raise ValueError(
                "Service not properly configured. Missing UI selectors. "
                "Please run discovery first."
            )
        
        # Build chat configuration
        chat_selectors = ChatUISelectors(
            input_selector=input_selector,
            send_button_selector=submit_selector,
            output_selector=output_selector,
            stop_button=ui_selectors.get("stop_button"),
            new_chat_button=ui_selectors.get("new_chat_button"),
        )
        
        chat_config = ChatConfig(
            url=service_config.get("url", ""),
            selectors=chat_selectors,
            timeout_ms=service_config.get("timeout_ms", 120000),
        )
        
        # Execute chat
        result = await self._chat_executor.execute_chat(
            browser=browser,
            context_id=context_id,
            config=chat_config,
            message=message,
        ...
        if not result.success:
            raise Exception(f"Chat execution failed: {result.error}")
        
        duration_ms = int((time.time() - start_time) * 1000)
        
        self._log.info(
            "Chat completion executed successfully",
            execution_id=execution_id,
            duration_ms=duration_ms,
