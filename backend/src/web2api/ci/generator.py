    ...
    def __init__(self, templates_dir: str | Path | None = None) -> None:
        ...
        if self._templates_dir and self._templates_dir.exists():
            self._env = Environment(
                loader=PackageLoader("web2api", "templates"),
...
{env_vars}
        run: |
          web2api run {test_paths_str} {parallel_str} \\
            --environment ${{{{ env.WEB2API_ENVIRONMENT }}}} \\
            --output-format junit \\
...
variables:
  PYTHON_VERSION: "{context["python_version"]}"
  WEB2API_ENVIRONMENT: "{context["environment"]}"
{env_vars}
...
test:
  ...
  script:
    - web2api run {test_paths_str} {parallel_str}
        --environment $WEB2API_ENVIRONMENT
        --output-format junit
        --output-file results.xml
        --artifacts-dir ./artifacts
  rules:
...
report:
  ...
  script:
    - pip install web2api
    - web2api report --input results.xml --format html --output report.html
  artifacts:
    paths:
      - report.html
    expire_in: {context["artifacts_retention_days"]} days
...
{env_vars}
    ...
    stages {{
        stage('Setup') {{
            steps {{
                sh 'python -m pip install --upgrade pip && pip install web2api'
            }}
        }}

        stage('Test') {{
            steps {{
                sh 'web2api run {test_paths_str} {parallel_str} --environment $WEB2API_ENVIRONMENT --output-format junit --output-file results.xml --artifacts-dir ./artifacts'
            }}
        }}
    }}
...
variables:
  ...
  web2apiEnvironment: '{context["environment"]}'
{env_vars}
    ...
    jobs:
      - job: RunTests
        ...
        steps:
          ...
          - script: |
              web2api run {test_paths_str} {parallel_str} \\
                --environment $(web2apiEnvironment) \\
                --output-format junit \\
