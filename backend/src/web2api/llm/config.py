"""LLM configuration models for Web2API.
...
class ToolName(StrEnum):
    """Web2API tools that can use LLM enhancement."""
...
class LLMConfig(BaseModel):
    """Complete LLM configuration for Web2API."""
...
class LLMSettings(BaseSettings):
    ...
    Loads configuration from environment variables with WEB2API_LLM_ prefix.
    """

    model_config = SettingsConfigDict(
        env_prefix="WEB2API_LLM_",
...
def load_llm_config(
    ...
    standard_paths = [
        Path(".web2api/llm.yaml"),
        Path(".web2api/llm.yml"),
        Path("web2api-llm.yaml"),
        Path("web2api-llm.yml"),
    ]
    if not file_config:
...
def _build_config_from_dict(data: dict[str, Any]) -> LLMConfig:
    ...
    if "api_key" in endpoint_data:
        ...
        env_key = os.getenv("WEB2API_LLM_API_KEY", "")
